...
> **V2.1 engineering baseline completed：Sales Workspace Kernel、FastAPI prototype、Android workspace / Draft Review ID demo、Postgres / Alembic persistence baseline、Sales Workspace Postgres store 与 Draft Review audit persistence 均已进入 main。V2.1 product experience not completed yet：chat-first 产品理解、获客方向迭代、ConversationMessage / AgentRun trace 和 Runtime -> WorkspacePatchDraft 设计仍需补齐。**
...
后续应先完成 V2.1 chat-first product experience design。真正 V2.2 evidence / search / ContactPoint、正式 LangGraph、Android review history view 或 DB hardening 仍不自动开放。
