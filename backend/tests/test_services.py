...
def test_create_product_profile_sets_expected_defaults(db_session) -> None:
    ...
    assert current_run.runtime_metadata["prompt_version"] == "product_learning_llm_v1"
    assert current_run.runtime_metadata["llm_provider"] == "tencent_tokenhub"
    assert current_run.runtime_metadata["llm_model"] == "minimax-m2.5"
...
def test_process_product_learning_run_updates_same_profile(db_session) -> None:
    ...
    assert refreshed_run.runtime_metadata["prompt_version"] == "product_learning_llm_v1"
...
def test_process_product_learning_run_marks_failed_for_invalid_llm_json(
    db_session,
    monkeypatch,
) -> None:
    created = services.create_product_profile(
...
def test_enrich_product_profile_appends_notes_and_increments_round_index(db_session) -> None:
    ...
    assert second_enrich.runtime_metadata["prompt_version"] == "product_learning_llm_v1"
