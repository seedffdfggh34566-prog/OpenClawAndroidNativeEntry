...
本文档用于冻结 V2 planning baseline 的最小边界，避免后续 agent 或开发线程把 V2 直接扩展成 CRM、联系人抓取、自动触达或大规模爬虫系统。
...
V2 的目标是把 V1 的“获客方向分析”推进到第一批可核验的候选公司 / 机构，而不是替用户自动销售。
...
V2 若继续只做抽象分析，差异化不足；若直接进入联系人抓取和自动触达，范围、合规和工程风险过高。
...
- V2 与通用 LLM 客户端形成更清晰差异：结构化对象、来源证据、候选卡片和研究历史。
