...
架构层文档用于说明系统分层、runtime、backend infrastructure、Android / data / workspace 关系。
...
- 当前主线是 V3 Agent Sandbox-first Memory-native Sales Agent。
- LangGraph / LangChain 是优先 runtime 路线。
- Product Sales Agent runtime memory 可以开放保存推断、假设、策略和纠错。
- V3 初期默认是 sandbox working state，不冻结 formal object / Kernel / Draft Review 路径。
- Backend 初期是 runtime host、storage、trace、API surface。
