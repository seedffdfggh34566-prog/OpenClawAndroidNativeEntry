...
- 默认 `/v3/sandbox` turn path 已切换为 TokenHub native tool/function calling，agent 通过 session-scoped core memory blocks 和 memory tools 自编辑记忆，并用 `send_message` 产出用户可见回复。
