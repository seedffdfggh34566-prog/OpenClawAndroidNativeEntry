...
- `task_v2_sales_workspace_runtime_patchdraft_prototype.md`

该任务只做 deterministic Runtime PatchDraft prototype，不开放正式 LangGraph graph、真实 LLM、search/contact/CRM、Android write path 或 DB migration。
...
- Current task：`task_v2_sales_workspace_runtime_patchdraft_prototype.md`
...
| `task_v2_sales_workspace_runtime_patchdraft_prototype.md` | deterministic Runtime PatchDraft prototype | `in_progress` |
...
- 当前只开放 deterministic Runtime PatchDraft prototype；后续 Android 体验增强、persistence-backed API、正式 Runtime / LangGraph integration 必须单独创建 task 后再推进。
