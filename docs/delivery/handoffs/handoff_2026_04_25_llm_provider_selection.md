...
- 更新 `docs/delivery/tasks/task_v1_product_learning_llm_phase1.md`，补充腾讯云 TokenHub Token Plan 企业版、`minimax-m2.5` 默认模型、human API Key 前置动作与模型对照策略。
...
- 腾讯云 Token Plan 企业版当前可用模型中，`minimax-m2.5` 的综合单价预估最低，适合作为第一轮可复现 baseline。
...
1. 查阅并引用腾讯云 Token Plan 企业版套餐概览、快速入门和 TokenHub 文本生成文档。
2. 确认企业版 OpenAI-compatible Base URL 为 `https://tokenhub.tencentmaas.com/plan/v3`。
3. 确认可用模型 ID 包含 `auto`、`kimi-k2.5`、`minimax-m2.5`、`glm-5`。
...
1. Human 在腾讯云 TokenHub Token Plan 企业版创建 dev API Key，并授权 `MiniMax-M2.5`，建议同时授权 `Kimi-K2.5` 与 `GLM-5`。
