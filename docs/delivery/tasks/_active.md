...
- 当前只开放 Sales Workspace repository layer v0，不开放 API store cutover、Android 或 Runtime。
...
- 当前唯一开放任务是 Sales Workspace repository layer v0：`docs/delivery/tasks/task_v2_sales_workspace_repository_layer_v0.md`。
...
当前长线程只开放 Sales Workspace repository layer v0；执行 agent 不应自动继续 API store cutover、Android 或 Runtime，除非当前 task 验证通过并进入本长线程计划的下一项。

Sales Workspace persistence migration v0 已完成。下一步执行 `task_v2_sales_workspace_repository_layer_v0.md`，不改变 API 默认 store。
