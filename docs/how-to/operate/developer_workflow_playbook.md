...
当前默认采用轻约束自治模型：用户或 `_active.md` 明确授权任务后，执行 agent 可以在边界内完成实现、验证、task 更新和 handoff。任务队列不清楚、方向变化或高风险操作需要停下确认。
...
4. [docs/product/prd/ai_sales_assistant_v3_prd.md](/home/yulin/projects/OpenClawAndroidNativeEntry/docs/product/prd/ai_sales_assistant_v3_prd.md)
5. [docs/adr/ADR-009-v3-memory-native-sales-agent-direction.md](/home/yulin/projects/OpenClawAndroidNativeEntry/docs/adr/ADR-009-v3-memory-native-sales-agent-direction.md)
6. [docs/architecture/v3/memory-native-sales-agent.md](/home/yulin/projects/OpenClawAndroidNativeEntry/docs/architecture/v3/memory-native-sales-agent.md)
7. [docs/delivery/tasks/_active.md](/home/yulin/projects/OpenClawAndroidNativeEntry/docs/delivery/tasks/_active.md)
...
- `closeout`：用于收口和同步 package / task 结果。
...
| Level 3：status / direction | capability status、project phase 或正式导航变化 | `project_status.md`、必要入口 README |

默认规则：

- 小 polish 或 package 内部 step 不默认更新 `docs/README.md`、`docs/delivery/README.md` 或 `docs/product/project_status.md`。
- 只有 package closeout、执行授权变化、队列状态变化时，才更新 `_active.md`。
- 只有 capability status 或 project phase 发生变化时，才更新 `project_status.md`。
- 只有文档导航、正式入口、目录结构或高层项目口径变化时，才更新 root / docs README。
- 如果 task 明确要求更新某个高层文档，可以更新，但必须在 task scope 或 handoff 中说明原因。

### 5.2.3 完成表述规则

task / handoff 只记录具体任务或 package 的完成情况、验证结果、已知限制和下一步。普通 task / handoff 不应自行判断产品阶段、版本、product experience 或 milestone 已完成。

允许写：

- 本 task 已完成。
- 本 package 已完成。
- 本次验证通过指定 smoke / test / docs check。
- 本 handoff 记录了本任务的证据和限制。

不允许在普通 task / handoff 中写：

- V3 已完成。
- V3 product experience completed。
- 当前产品阶段已完成。
- 下一步只能进入某个新版本。

如果确实需要判断某个产品阶段或版本状态，必须由用户明确要求并单独创建 review / status 文档，不要把这个判断塞进普通 task 或 handoff。
...
1. 更新 task 状态
2. 更新或新增 handoff
3. 按影响范围决定是否更新 `_active.md`、`project_status.md` 或入口 README
