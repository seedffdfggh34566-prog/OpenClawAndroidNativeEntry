...
在 backend 实现前收口任务边界、验证路径和 handoff，特别适用于 V3 runtime、memory、sandbox working state、customer intelligence、API 和 persistence 工作。

## When to trigger

- backend 非 trivial 任务没有明确 task
- 当前请求是 follow-up，不适合继续污染已完成 task
- 触及 API contract、runtime boundary、memory、sandbox working state、customer intelligence、persistence、migration、observability 或环境假设
...
- 需要改产品方向或把 sandbox working state 变成 formal object 语义
