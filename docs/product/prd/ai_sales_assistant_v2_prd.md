...
- 文档状态：Draft v0.4
- 更新日期：2026-04-29
- 阶段定位：V2 historical prototype definition；当前方向已被 V3 Memory-native Sales Agent supersede
- 关联文档：
...
> 2026-04-29 更新：当前项目方向已正式切换为 V3。本文档保留为 V2 Sales Workspace / Kernel prototype 的历史参考，不作为当前默认开发方向。
...
### 2.1 Product Sales Agent 认知记忆方向

V2 的 Product Sales Agent 不应被限制为只记住“已确认事实”。后续 runtime 方向允许 agent 维护可自编辑的长期认知记忆，其中可以包含用户原话、合理推断、暂定假设、销售策略、纠错记录和已废弃信息。

这些记忆应通过状态标记、审阅和正式写回边界治理，而不是在进入记忆前一律禁止。正式业务对象、对外承诺、联系方式和报告仍必须经过 backend services / Sales Workspace Kernel 裁决。

当前倾向使用 LangGraph / LangChain 作为自研 runtime 路线，并借鉴 Letta 的 memory blocks、archival memory 和 memory tools 模式。该方向不冻结具体 schema、API 或 graph 实现。
...
- 搜索 provider。
- Product Sales Agent memory 的最小对象、工具和持久化方式。
...
- 用户是否能通过一个轻量入口开始销售工作区，并在进入后用自然语言表达业务。
...
- 用户首次使用时无需理解 workspace 概念，可点击轻量入口“开始销售工作区”进入聊天；系统在后台创建或进入默认 `SalesWorkspace`。
- 用户进入销售工作区后能通过自然语言表达业务，并沉淀为 `ConversationMessage` / `AgentRun`。
