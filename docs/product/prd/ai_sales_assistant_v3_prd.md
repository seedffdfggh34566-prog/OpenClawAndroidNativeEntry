...
- 更新日期：2026-04-30
- 阶段定位：V3 Memory-native Sales Agent 方向定义；implementation not started
- 关联文档：
...
> **Memory-native Sales Agent：Product Sales Agent 拥有可自编辑认知记忆，基于 LangGraph / LangChain 自研 runtime 持续经营 Sales Workspace；backend 只在正式业务承诺处做治理。**

V3 不再把 Product Sales Agent 主要定义为 `WorkspacePatchDraft` 生成器，也不再把“只允许确认事实进入记忆”作为默认策略。V3 允许 agent 形成、修正和使用长期认知状态，再通过标记、审阅和正式写回边界治理风险。
...
- Backend / Sales Workspace Kernel 仍负责正式业务对象、正式报告、联系方式和对外动作的治理。
...
- 推断和假设可以被保存，但不会被误写成 confirmed formal object。
- 用户纠错会影响后续回答和策略。
- 正式业务对象写回仍经过 backend governance。
- 腾讯云模型 API 可通过 LangChain 或 OpenClaw LLM Gateway 稳定调用。
- Web `/lab` 能支持人工测试和 Dev Agent 自动化验证 memory、trace、draft review 与 formal writeback boundary。
...
V3 继承 V2 的 Sales Workspace 资产，但当前默认开发方向转为 memory-native Sales Agent。旧 V2 文档保留为 historical validated prototype / reference only，不再作为当前默认产品方向。
