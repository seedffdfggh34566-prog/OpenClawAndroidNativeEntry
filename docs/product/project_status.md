...
| V3 Web dual-entry direction | `lab_scaffold_completed / workspace_not_started` | 已有 `/lab` 内部测试入口；`/workspace` 用户雏形未实现，App 仍是长期主要入口。 |
...
| V3 Web Lab / Workspace prototype | `lab_scaffold_completed` | `task_2026_04_30_v3_web_lab_scaffold.md`。 | 后续可开放 replay/seed 或 `/workspace` task。 |
